	...
	{			
		...
		public function setEstadoCobranza(estado:String):String{
			...
			switch(estado){
				case "1":
					return "Normal";
				case "2":
					return "10 días de mora";
				case "3":
					return "20 días de mora";
				case "4":
					return "30 días de mora";
				case "5":
		...
		public function buscar(paginador:Paginador, buscarPor:String=""):void{			
			...
			if(this.buscarPor == CUENTA){
				typeEvent = ClientesGeneralEvent.BUSCAR_CLIENTES_CUENTA;
			} else {
				typeEvent = ClientesGeneralEvent.BUSCAR_CLIENTES;
			}
			var evt:ClientesGeneralEvent = new ClientesGeneralEvent(typeEvent, filtro, paginador);
			evt.dispatch();
		}
		
		public function setearCliente(cliente:ClienteTransaccion):void
		{
			if (ModelLocatorGeneral != null)
			{
				/*ModelLocatorGeneral.getInstance().clienteSeleccionado.cliente = cli;
				ModelLocatorGeneral.getInstance().clienteSeleccionado.futurosVencimientos.removeAll();*/
				//(isNaN(cliente.idTitular)?this.cliente = cliente:this.cliente = titular)
				this.cliente = cliente;
				//Alert.show("Nro " + cliente.idCliente);
				this.futurosVencimientos.removeAll();
			}
		}
		public function setearTitular(titular:ClienteTransaccion):void
		{
			if (ModelLocatorGeneral != null)
			{
				/*ModelLocatorGeneral.getInstance().clienteSeleccionado.cliente = cli;
