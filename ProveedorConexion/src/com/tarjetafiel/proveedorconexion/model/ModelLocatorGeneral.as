	...
	public class ModelLocatorGeneral implements IModelLocator {
		...
		{												//abrirá el iframe del modulo verDocAdj
			...
			if((timestamp.month+1)<10){
			 ...
			 amazonS3 = "https://s3-sa-east-1.amazonaws.com/tarjetafiel/tramites";		
			
			
			if  (doc.url.indexOf("/lineaCredito/") > -1) {
				
				urlDocAdj = amazonS3    + doc.url;
			} else if  (doc.url.indexOf("-AmazonAWS-") > -1) {
				urlDocAdj = amazonS3  + "/"  + año + "-" + mes +  "/" + doc.url;
			}else if(timestamp<fecha1){
