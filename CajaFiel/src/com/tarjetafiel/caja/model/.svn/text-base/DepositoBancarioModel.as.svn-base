package com.tarjetafiel.caja.model
{
	...
	[Bindable]
	public class DepositoBancarioModel extends EventDispatcher
	{
		...
		public function addMedioPago():void{
			addCheque(); 
			movimMp.formaPago = ModelLocator.getInstance().formaPagoCajaModel.getFormaPago(6);
		...
		public function set movimMpOld(value:MovimientoMP):void{
			...
			modificar = true;
			dispatchEvent(new Event("changedMovimientoMpOld"));
		}
		
		[Bindable (event="changedMovimientoMpOld")]
		public function get movimMpOld():MovimientoMP{
			return _movimMpOld;
		}
		
		
		[Bindable (event="changedMovimMp")]
