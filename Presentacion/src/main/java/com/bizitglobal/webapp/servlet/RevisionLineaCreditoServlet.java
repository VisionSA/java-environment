...
import com.bizitglobal.tarjetafiel.evaluacion.service.impl.DigitalServiceImpl;
import com.bizitglobal.tarjetafiel.evaluacion.service.impl.IndividuoEvaluacionServiceImpl;
import com.bizitglobal.tarjetafiel.general.negocio.TipoDigital;
import com.bizitglobal.tarjetafiel.general.service.impl.TipoDigitalServiceImpl;
...
public class RevisionLineaCreditoServlet extends HttpServlet implements javax.servlet.Servlet {

  /**
	 * 
	 */
	private static final long serialVersionUID = -9166416935188731214L;
	private Logger log = Logger.getLogger(RevisionLineaCreditoServlet.class);
	...
	public RevisionLineaCreditoServlet() {
		super();
	}
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse resp)
	  ...
	  try{
		  ...
		  DigitalServiceImpl digitalService = (DigitalServiceImpl) appContext.getBean("digitalServiceTarget");
		  IndividuoEvaluacionServiceImpl individuoEvaluacionService = (IndividuoEvaluacionServiceImpl) appContext.getBean("individuoEvaluacionServiceTarget");
		  TipoDigitalServiceImpl tipoDigitalService = (TipoDigitalServiceImpl) appContext.getBean("tipoDigitalServiceTarget");
...
//		  int diaCierre= Integer.parseInt(obj.get("diaCierre").toString()); 
		  long idIndividuo= Long.parseLong(obj.get("idIndividuo").toString());
		  
		  
		  
		  String ingresos = (obj.get("ingresos").toString().trim());
		  String empleador = (obj.get("empleador").toString().trim());
		  String observaciones = (obj.get("observaciones").toString().trim());
		  long idCliente = Long.parseLong(obj.get("idCliente").toString());
		  
		  JSONArray documentos = (JSONArray) obj.get("documentos");
		  
		  IndividuoEvaluacion individuoEvaluacion = individuoEvaluacionService.leerIndividuo(idIndividuo);
		  TipoDigital tipoDigital = tipoDigitalService.leerTipoDigital(19L);
		  
		  		  
		  for(int i = 0; i<documentos.length(); i++){
			  Digital digital = new Digital();
			  digital.setIndividuoEvaluacion(individuoEvaluacion);
//			  digital.setUrl(documentos.get(i).toString());
			  digital.setUrl(documentos.get(i).toString().replace("https://tarjetafiel.s3.sa-east-1.amazonaws.com/tramites",""));
			  digital.setIdOperador(1262L);
			  digital.setTipoDigital(tipoDigital);
			  digital.setTimestamp(new Timestamp(Calendar.getInstance().getTime().getTime()));
			  digital.setDescripcion("APP");
			  digitalService.grabarDigital(digital);
		  ...
		  BloqueoPlastico bloqueoPlastico = new BloqueoPlastico();
		  
		  bloqueoPlastico.setCuil(individuoEvaluacion.getCuil());
		  bloqueoPlastico.setIdTitular(idCliente);
		  bloqueoPlastico.setApellidoNombres(individuoEvaluacion.getApellido() + " " + individuoEvaluacion.getNombres());
		  bloqueoPlastico.setNroDocumento(individuoEvaluacion.getNroDocumento());
		  String textoObs = "";
		   textoObs += ((ingresos.equals("")) ? "" :  "Ingresos: "+ingresos);
		   textoObs += ((empleador.equals("")) ? "" :  ",Empleador: " + empleador);
		   textoObs += ((observaciones.equals("")) ? "" : ",Observacion: " + observaciones);
		  
		  bloqueoPlastico.setObservacionTexto(textoObs);
				...
				Filtro filtro = new Filtro(Proceso.TITULO, Filtro.LIKEXS, "Revision Linea Credito Reducida");
		...
		if (!parametroWrapperList.isEmpty()) {
			...
			while (iterator.hasNext()) {
				...
				if (wrapper.getAtributo().getNombre().equals("Base")) {
					...
					log.info("Base --> " + wrapper.getValorTexto());
				}
				...
				if (wrapper.getAtributo().getNombre().equals("nota_cred")) {
					...
					log.info("nota_cred --> " + wrapper.getValorTexto());
				}
			...
			while (iterator.hasNext()) {
				...
				if (!wrapper.isCompletaPanel()) {
					if (wrapper.getAtributo().getNombre().equals("nroCuenta")) {
						atributosMap.put(wrapper.getAtributo().getNombre(), bloqueoPlastico.getIdTitular());
						wrapper.setValorNroEntero(new Integer(bloqueoPlastico.getIdTitular().toString()));
						log.info("Base2 --> " + wrapper.getValorTexto());
					}
					if (wrapper.getAtributo().getNombre().equals("tipoIndividuo")) {
						atributosMap.put(wrapper.getAtributo().getNombre(), 1);
						wrapper.setValorTexto(1 + "");
					...
					if (wrapper.getAtributo().getNombre().equals("nota_cred")) {
						atributosMap.put(wrapper.getAtributo().getNombre(), bloqueoPlastico.getObservacionTexto());
						wrapper.setValorTexto(bloqueoPlastico.getObservacionTexto());
						log.info("nota_cred2 --> " + wrapper.getValorTexto());
					}
