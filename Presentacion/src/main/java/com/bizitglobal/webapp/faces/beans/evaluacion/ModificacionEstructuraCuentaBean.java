...
import com.bizitglobal.tarjetafiel.transacciones.exception.GaranteException;
...
import com.bizitglobal.tarjetafiel.transacciones.negocio.Garante;
...
public class ModificacionEstructuraCuentaBean extends BaseBean {
	private static final Logger log = Logger.getLogger(ModificacionEstructuraCuentaBean.class);
	...
	private List listGaranteBaja = new ArrayList();
	private List listAdicionalBaja = new ArrayList();
	...
	/* @I4991 */private boolean deshabilitarProfesion;
	/* @F4991 */
	...
	private boolean buscarIndividuo;


	public ModificacionEstructuraCuentaBean() {
		super();
	}
	...
	public void borrar() {
		...
		opcionA = false;
		esCBU = false;
		/* @I4991 */deshabilitarProfesion = true;
		/* @F4991 */habilitarSucursal = true;
	...
	public void inicializarParametros(Map param) {
		...
		try {
			...
			error.agregar(Error.EVA_NUMERO_SOLICITUD_INCORRECTO + " " + e.getMessage());
	...
	public String inicializar() {
		log.info("Entrando a modificacionEstructuraCuenta");
		return null;
	}
	...
	public void cargarDatosMinimosNecesarios() {
		...
		/* @I4991 */individuoEvaluacion.setVinculo(null);
		/* @F4991 */bancos.setIndividuo(individuoEvaluacion);
	 ...
	 * Adicionales
	 */
	public String inicializar(int origen, String nroSolicitud, Long idIndi) {

		String resultado = "modificacionEstructuraCuenta";
		if (error.cantidad() != 0)
			return null;
		listAdicional = new ArrayList();
		listGarante = new ArrayList();
		...
		try {
			this.clienteTransaccion = transaccionesService.getClienteTransaccionService().leerCliente(Long.parseLong(this.nroCuenta));
			this.nroCuit = clienteTransaccion.getIndividuo().getCuil();
		} catch (NumberFormatException e) {
			e.printStackTrace();
		} catch (ClienteTransaccionException e) {
			e.printStackTrace();
		}
		IndividuoEvaluacion indiEva = clienteTransaccion.getIndividuo();
		if (indiEva.getDomicilio() != null && indiEva.getDomicilio().validateDomicilio())
		{
			// Si la solicitud individuo de la cuenta que inicia el tramite de modificacion estructura cuenta no existe, la crea.
			crearSolicitudIndividuoEvaluacion(this.nroSolicitud, this.nroCuit);

			String result = cargarSolicitud(new Long(this.nroSolicitud));
			...
			if (idDomPagoSeleccionado != new Long(0)) {
				...
				if (!idPart.equals(new Long(0))) {
					listDiaPago.addAll(Util.cargarSelectItem(
							evaluacionService.getDiaPagoService().getDiaPago(new Filtro("partido.idPartido", Filtro.IGUAL, idPart))));
				}
			}
	...
	public String agregarDomicilioInmueble() {
		...
		bean.inicializar(DomicilioBean.GARANTIA_MODIFICAR_CUENTA, new Domicilio());
	...
	public String agregarDomicilioPopup() {
		...
		switch (origen) {
		case TITULAR:
			/* @I4991 */if (individuoEvaluacion.getDomicilio() == null)
				individuoEvaluacion.setDomicilio(new Domicilio());
			/* @F4991 */bean.inicializar(DomicilioBean.TITULAR_MODIFICAR_CUENTA, individuoEvaluacion.getDomicilio());
			break;
		case GARANTE:
			/* @I4991 */if (individuoEvaluacion.getDomicilio() == null)
				/* @F4991 */individuoEvaluacion.setDomicilio(new Domicilio());
	...
	public String agregarTelefono() {
		...
		bean.inicializar(TelefonoBean.MODIFICACION_ESTRUCTURA_CUENTA, new Telefono());
	...
	public String editarTelefono() {
		...
		bean.inicializar(TelefonoBean.MODIFICACION_ESTRUCTURA_CUENTA, telefono);
	...
	public boolean validar() {
		...
		// No se usa mas ya que los numeros de solicitud son fincticios para este tramite.
	...
	public void validarDatosRequeridos() {
		...
		if (origen == TITULAR || origen == GARANTE)
		{
			try
			{
				if (individuoEvaluacion.getActividad() == null
						|| individuoEvaluacion.getActividad().getSucEmpresa() == null
						|| individuoEvaluacion.getActividad().getSucEmpresa().getIdSucEmpresa().equals(new Long(0)))
	...
	public String cargarSolicitud(Long idAdi) {
		...
		if (validar()) {
			...
			try {
				...
				filtro = new Filtro("nroSolicitud", Filtro.LIKEXS, nroSolicitud.substring(0, nroSolicitud.length() - 1));
				solicitud = null;
				...
				if (solicitud != null) {
					if (solicitud.getPromotor() != null) {
						...
						/* @I4991 */listGarante.clear();
						listAdicional.clear();
						/* @F4991 */// Se cargan los garantes y adicionales en la solicitud para edicion o baja.
						this.cargarIndividuosRelacionadosTitular();

						Long id = solicitud.getIdSolicitud();
						/* @I4991 */
						/* @F4991 */Iterator iterCargando = solicitud.getSolicIndividuos().iterator();
						while (iterCargando.hasNext()) {
							...
							if (sol.getTipoIndividuo().getIdTipoIndividuo().equals(new Long(2)) && (sol.getActivo().compareTo("S") == 0
									|| sol.getNuevo().equals("N"))) {

								listGarante.add(new WrapperIndividuoEvaluacion(in, sol.getNuevo(), sol.getActivo()));
								...
								if (sol.getTipoIndividuo().getIdTipoIndividuo().equals(new Long(3)) && (sol.getActivo().compareTo("S") == 0
										|| sol.getNuevo().equals("N"))) {
									listAdicional.add(new WrapperIndividuoEvaluacion(in, sol.getNuevo(), sol.getActivo()));
						...
						if (!solicitudesIndividuos.isEmpty()) {
							...
							while (iterator.hasNext()) {
								...
								if (solicitudIndividuo.getTipoIndividuo().getIdTipoIndividuo().equals(new Long(2))
									...
									listGarante.add(new WrapperIndividuoEvaluacion(individuoEvaluacion, solicitudIndividuo.getNuevo(),
											solicitudIndividuo.getActivo()));
									break;
								}
							...
							IndividuoEvaluacion individuoEvaluacion = buscarIndividuoPorCuit(this.nroCuit);
							if (individuoEvaluacion != null) {
	...
	private void cargarIndividuosRelacionadosTitular()
	{
		/* @F4991 */try {
			// *********** Se genera una SolicitudIndivuduo por cada garante de la cuenta del titular ****************//
			if (evaluacionService.getSolicitudIndividuoService().isCargarGarantes(solicitud)) {
				Filtro filtroSolic = new Filtro();
				filtroSolic.agregarCampoOperValor("cliente.idCliente", Filtro.IGUAL, this.clienteTransaccion.getIdCliente());
				filtroSolic.agregarCampoOperValor("activo", Filtro.IGUAL, "'S'");
				List listGaran = null;
				try {
					listGaran = transaccionesService.getGaranteService().getGarante(filtroSolic);
				} catch (GaranteException e1) {
					e1.printStackTrace();
				}

				Iterator iterGarantes = listGaran.iterator();
				while (iterGarantes.hasNext())
				{
					Garante garante = (Garante) iterGarantes.next();
					try {
						solicitudIndividuo = new SolicitudIndividuo();
						solicitudIndividuo.setIdSolicitudIndividuo(null);
						solicitudIndividuo.setSolicitud(solicitud);
						solicitudIndividuo.setTipoIndividuo(new TipoIndividuo(new Long(2)));
						solicitudIndividuo.setAceptado("S");
						solicitudIndividuo.setActivo("S");
						solicitudIndividuo.setNuevo("N");

						IndividuoEvaluacion indi = evaluacionService.getIndividuoEvaluacionService().leerIndividuo(
								garante.getIndividuo().getIdIndividuo());
						solicitudIndividuo.setIndividuoEvaluacion(indi);
						...
						e.printStackTrace();
					} catch (SolicLogException e) {
						e.printStackTrace();
					}
				}
			}

			// *********** Se cargan los adicionales en la solicitud del individuo titular ****************//
			if (evaluacionService.getSolicitudIndividuoService().isCargarAdicionales(solicitud)) {
				Filtro filtroAdic = new Filtro();
				filtroAdic.agregarCampoOperValor("idTitular", Filtro.IGUAL, new Long(this.nroCuenta));
				filtroAdic.agregarCampoOperValor("estadoCliente.idEstadoCliente", Filtro.DISTINTO, 4L);
				List listAdic = null;
				try {
					listAdic = transaccionesService.getClienteTransaccionService().getCliente(filtroAdic);
				} catch (ClienteTransaccionException e) {
					e.printStackTrace();
				}

				Iterator iterAdicionales = listAdic.iterator();
				while (iterAdicionales.hasNext())
				{
					ClienteTransaccion adicional = (ClienteTransaccion) iterAdicionales.next();
					try {
						solicitudIndividuo = new SolicitudIndividuo();
						solicitudIndividuo.setIdSolicitudIndividuo(null);
						solicitudIndividuo.setSolicitud(solicitud);
						...
						IndividuoEvaluacion indi = adicional.getIndividuo();
						solicitudIndividuo.setIndividuoEvaluacion(indi);
						solicitud.getSolicIndividuos().add(solicitudIndividuo);
	...
	public void recargarIndividuosRelacionados()
	{
		try {
			filtro = new Filtro("nroSolicitud", Filtro.LIKEXS, nroSolicitud.substring(0, nroSolicitud.length() - 1));
			solicitud = null;
	...
	public String traerIndividuo() {
		...
		try {
			...
			if (!listaDeIndividuosExis.isEmpty()) {
				...
				f.agregarCampoOperValor("solicitud.nroSolicitud", Filtro.LIKEXS, nroSolicitud.substring(0, nroSolicitud.length() - 1));
				f.agregarCampoOperValor("solicitud.DV", Filtro.LIKEXS, "0");
				...
				if (solicIndiv.isEmpty()) {
					...
					if (origen == TITULAR) {
						...
						inicializar(1, nroSolicitud, null);
						Session.redirect("/tarjetafiel/evaluacion/modificacionEstructuraCuenta.jsf");
					}
					if (origen == GARANTE) {
					...
					if (origen == 3) {
						...
						inicializar(3, nroSolicitud, null);
						Session.redirect("/tarjetafiel/evaluacion/modificacionEstructuraCuenta.jsf");
	...
	/*
	 * Este metodo se usa para crear la solicitud individuo,del cliente que se esta modificando su estructura de cuenta. Tambien setea el
	 * tipoSolicitud=2 (Modificacion Estructura cuenta)
	 */
	public String crearSolicitudIndividuoEvaluacion(String nroSolicitud, String cuitIndividuo) {
		verCargaIndividuo = false;
		try {
			...
			if (!solicitud.getIdTipoSolicitud().equals(new Long(2)))
			{
				solicitud.setIdTipoSolicitud(new Long(2));
				evaluacionService.getSolicitudService().actualizarSolicitud(solicitud);
			}
			if (solicitud.getSolicIndividuos().isEmpty()) {
				cuil = this.nroCuit;
				log.info("Alta de Modificacion Estructura Cuenta titular sin individuo existente");
				alta = true;
				this.individuoEvaluacion = buscarIndividuoPorCuit(cuitIndividuo);
				yaExiste = false;
				habilitada = false;
				TipoIndividuo tip = null;
				try {
					tip = evaluacionService.getTipoIndividuoService().leerTipoIndividuo(new Long(1));
				} catch (TipoIndividuoException e) {
					e.printStackTrace();
				}
				solicitudIndividuo = new SolicitudIndividuo();
				solicitudIndividuo.setIdSolicitudIndividuo(null);
				solicitudIndividuo.setTipoIndividuo(tip);
				solicitudIndividuo.setIndividuoEvaluacion(this.individuoEvaluacion);
				solicitudIndividuo.setSolicitud(solicitud);
				solicitudIndividuo.setIdSolicitudIndividuo(null);
				solicitudIndividuo.setAceptado("S");
				solicitudIndividuo.setActivo("S");
	...
	public String buscarOtroIndividuo() {
		...
		// break;
		// }
		return "modificacionEstructuraCuenta";
	}
		...
		if (elemento.getDiaPago() != null) {
			...
			if (elemento.getDiaPago().getPartido() != null) {
				...
				if (elemento.getDiaPago().getPartido().getIdPartido() != null
					...
					try {
						listAuxDiaPago = evaluacionService.getDiaPagoService().getDiaPago(new Filtro());
	...
	public void cambiarProfesion(ValueChangeEvent event) {
		/* @I4991 */if (educacionHtml != null && educacionHtml.getValue() != null && !educacionHtml.getValue().equals("")) {
			if (new Long(educacionHtml.getValue().toString()).equals(new Long(8))) {
				deshabilitarProfesion = false;
				focoHidden = "lstPorfesion";
			} else {
				deshabilitarProfesion = true;
				focoHidden = "lstPorfesion";
			}
		} else {
			deshabilitarProfesion = false;
	...
	public String guardarSolicitud() {
		...
		if (validarGuardar()) {
			...
			try {
				...
				if (solicitudIndividuo.getIdSolicIndividuo() != null) {
					/* @I4991 */corregirSolicitudIndividuo();
					/* @F4991 */evaluacionService.getSolicitudIndividuoService().actualizarSolicitudIndividuo(solicitudIndividuo);
					...
					if (solicitud != null) {
						...
						// solicitudIndividuo.setSolicitud(solicitud);
						/* @I4991 */corregirSolicitudIndividuo();
						/* @F4991 */evaluacionService.getSolicitudIndividuoService().grabarSolicitudIndividuo(solicitudIndividuo);
						evaluacionService.getSolicLogService().grabarSolicLog(
								new SolicLog(solicitud.getEstados(), Session.getOperador().getCodigo(), solicitudIndividuo));
					}
					alta = false;
				...
				// }
				/* @I4991 */
				/* @F4991 */if (individuoEvaluacion.getTipoDocumento() == null) {
	...
	/* @I4991 */private void corregirSolicitudIndividuo() {
		if (objetoNegocioInconsistente(solicitudIndividuo.getIndividuoEvaluacion().getEducacion())) {
			solicitudIndividuo.getIndividuoEvaluacion().setEducacion(null);
		}
		if (objetoNegocioInconsistente(solicitudIndividuo.getIndividuoEvaluacion().getProfesion())) {
			solicitudIndividuo.getIndividuoEvaluacion().setProfesion(null);
		}
		if (objetoNegocioInconsistente(solicitudIndividuo.getIndividuoEvaluacion().getEstadoCivil())) {
			solicitudIndividuo.getIndividuoEvaluacion().setEstadoCivil(null);
		}
		if (objetoNegocioInconsistente(solicitudIndividuo.getIndividuoEvaluacion().getVinculo())) {
			solicitudIndividuo.getIndividuoEvaluacion().setVinculo(null);
		}
		/*
		 * if (objetoNegocioInconsistente(solicitudIndividuo.getIndividuoEvaluacion().getActividad()) ){
		 * solicitudIndividuo.getIndividuoEvaluacion().setActividad(null); }
		 */
	...
	private void armarIndividuo() {
		...
		if (solicitudIndividuo.getIndividuoEvaluacion() != null) {
			...
			/* @I4991 */if (solicitudIndividuo.getTipoIndividuo() != null
					&& new Long(1).equals(solicitudIndividuo.getTipoIndividuo().getIdTipoIndividuo())) {
				individuoEvaluacion.setVinculo(null);
			}
			/* @F4991 */if (idTipoCuentaSeleccionado != null && !idTipoCuentaSeleccionado.equals(new Long(0))) {
			...
			armarActividad();
			/* @I4991 */
			/* @F4991 */individuoEvaluacion.setTimestamp(new Timestamp(new java.util.Date().getTime()));
	...
	public String altaGarante() {
		...
		alta = true;
		this.setBuscarIndividuo(true);
		mostrarEducacion = true;
		if (this.validarListGarantes(listGarante)) {
			String nroSolicitudAuxiliar = nroSolicitud;
			nuevaSolicitud();
			yaExiste = false;
			habilitada = true;
	...
	public String altaAdicional() {
		...
		/* @I4991 */if (validarAltaAdicional()) {
			/* @F4991 */// guardarSolicitud();
			mostrarEducacion = false;
			alta = true;
			String nroSolicitudAuxiliar = nroSolicitud;
			nuevaSolicitud();
	...
	public String abrirArchivo() {
		...
		try {
			...
			ejecutarJavaScript("popup('" + "/../archivos/" + Archivo.archivosDeIndividuos + "/" + element.getNombreArchivo() + "',700,400);");
		} catch (Exception e) {
	...
	public String buscarIndividuo() {
		...
		if (buscarIndividuoPorCuit(this.nroCuit) != null) {
			individuoEvaluacion = buscarIndividuoPorCuit(this.nroCuit);
		}
		return null;
	}


	public IndividuoEvaluacion buscarIndividuoPorCuit(String pNroCuit) {
		IndividuoEvaluacion result = null;
		try {
			if (pNroCuit != null && !pNroCuit.equals("")
					&& pNroCuit.length() == 11) {
				if (validarCuit(pNroCuit)) {

					List individuos = evaluacionService
							.getIndividuoEvaluacionService().getIndividuo(
									new Filtro("cuil", Filtro.LIKEXS, pNroCuit));
	...
	public String editarGarante() {
		/* @I4991 */// guardarSolicitud();
		Long idGarante = new Long(Session.getRequestParameter("idGaranteEdi"));
		Long numeroSol = null;
		for (Object obj : solicitud.getSolicIndividuos()) {
			solicitudIndividuo = (SolicitudIndividuo) obj;
			if (solicitudIndividuo.getIndividuoEvaluacion().getIdIndividuo().equals(idGarante)) {
		...
		vista = 1;
		individuoEvaluacion = solicitudIndividuo.getIndividuoEvaluacion();
		/* @I4991 */inicializar(2, nroSolicitud, numeroSol);
		/* @F4991 */Session.redirect("/tarjetafiel/evaluacion/modificacionEstructuraCuenta.jsf");
		return null;
	...
	public void eliminarGarante() {
		/* @I4991 */Long idGarante = new Long(Session.getRequestParameter("idGaranteElim"));
		log.info("id a buscar: " + idGarante);
		List solicitudesIndiv = new ArrayList();
		solicitud = this.recargarSolicitud(nroSolicitud);
		solicitudesIndiv = Convertidores.setToList(solicitud.getSolicIndividuos());
		SolicitudIndividuo solicitudIndividuo = null;
		if (!solicitudesIndiv.isEmpty()) {
			Iterator iterator = solicitudesIndiv.iterator();
			while (iterator.hasNext()) {
				solicitudIndividuo = (SolicitudIndividuo) iterator.next();
				if (solicitudIndividuo.getTipoIndividuo().getIdTipoIndividuo().equals(new Long(2))
						&& solicitudIndividuo.getIndividuoEvaluacion().getIdIndividuo().equals(idGarante)) {
					log.info("SolicitudIndividuoEncontrada");
					break;
				}
			}
		}
		listGarante.remove(solicitudIndividuo.getIndividuoEvaluacion());
		try {
			if (solicitudIndividuo.getActivo().equals("S")) {
				solicitudIndividuo.setActivo("N");
			} else {
				solicitudIndividuo.setActivo("S");
			}
			/* @F4991 */evaluacionService.getSolicitudIndividuoService().actualizarSolicitudIndividuo(solicitudIndividuo);
			this.recargarIndividuosRelacionados();
		} catch (SolicitudIndividuoException e) {
			e.printStackTrace();
		}
	}
	...
	public void eliminarGaranteBaja() {
		List solicitudesIndiv = new ArrayList();
		solicitudesIndiv = Convertidores.setToList(solicitud.getSolicIndividuos());
		if (!solicitudesIndiv.isEmpty()) {
			Iterator iterator = solicitudesIndiv.iterator();
	...
	public void eliminarAdicional() {
		...
		solicitud = this.recargarSolicitud(nroSolicitud);
		solicitudesIndiv = Convertidores.setToList(solicitud.getSolicIndividuos());
		/* @I4991 */SolicitudIndividuo solicitudIndividuo = null;
		/* @F4991 */if (!solicitudesIndiv.isEmpty()) {
			Iterator iterator = solicitudesIndiv.iterator();
			while (iterator.hasNext()) {
				solicitudIndividuo = (SolicitudIndividuo) iterator.next();
				if (solicitudIndividuo.getTipoIndividuo().getIdTipoIndividuo().equals(new Long(3))
		...
		try {
			/* @I4991 */if (solicitudIndividuo.getActivo().equals("S")) {
				solicitudIndividuo.setActivo("N");
			} else {
				solicitudIndividuo.setActivo("S");
			}
	...
	public Solicitud recargarSolicitud(String nroSolicitud)
	{
		Solicitud solic = null;
		filtro = new Filtro("nroSolicitud", Filtro.LIKEXS, nroSolicitud.substring(0, nroSolicitud.length() - 1));
		try {
			solic = evaluacionService.getSolicitudService().busNroComprobante(filtro);
		} catch (SolicitudException e) {
			e.printStackTrace();
		}
	...
	public String editarAdicional() {
		...
		/* @I4991 */IndividuoEvaluacion indi = solicitudIndividuo.getIndividuoEvaluacion();
		inicializar(3, nroSolicitud, null);
		...
		if (!alta) {
			try {
				...
				if (sol != null
					...
					Session.redirect("/tarjetafiel/evaluacion/popup/buscarIndividuoModificacionCuenta.jsf");
				}
	...
	public Long getIdTipoSexoSeleccionado() {
		/* @I4991 */if (individuoEvaluacion.getSexo() == null) {
			return 0L;
		}
		else {
			if (individuoEvaluacion.getSexo().equals("F")) {
				return 1L;
			}
			else if (individuoEvaluacion.getSexo().equals("M")) {
				return 2L;
			}
			else
				return 0L;
	...
	public void setIdTipoSexoSeleccionado(Long idTipoSexoSeleccionado) {
		...
		/* @I4991 */if (new Long(1).equals(idTipoSexoSeleccionado)) {
			individuoEvaluacion.setSexo("F");
			this.idTipoSexoSeleccionado = idTipoSexoSeleccionado;
		}
		else if (new Long(2).equals(idTipoSexoSeleccionado)) {
			individuoEvaluacion.setSexo("M");
			this.idTipoSexoSeleccionado = idTipoSexoSeleccionado;
		}
		else {
			individuoEvaluacion.setSexo(null);
			this.idTipoSexoSeleccionado = 0L;
		}
		/* @F4991 */}
	...
	/* @I4991 */public boolean getDeshabilitarProfesion() {
		return deshabilitarProfesion
				&& !(individuoEvaluacion.getEducacion() != null && individuoEvaluacion.getEducacion().getIdEducacion().equals(8L));
	}


	public void setDeshabilitarProfesion(boolean deshabilitarProfesion) {
		this.deshabilitarProfesion = deshabilitarProfesion;
	}


	/* @F4991 */public void setNombrePromotor(String nombrePromotor) {
	...
	/* @I4991 */public Long getIdEducSeleccionado() {
	...
	public void setIdEducSeleccionado(Long idEducSeleccionado) {
		individuoEvaluacion.setEducacion((Educacion) Util.buscarElemento(listAuxEducacion, idEducSeleccionado));
	}
	...
	public void setIdProfesionSeleccionado(Long idProfesionSeleccionado) {
		if (individuoEvaluacion.getEducacion() != null && individuoEvaluacion.getEducacion().getIdEducacion().equals(8L)) {
			individuoEvaluacion.setProfesion((Profesion) Util.buscarElemento(this.listAuxProfesion, idProfesionSeleccionado));
		} else {
	...
	public Long getIdVinculoSeleccionado() {
		if (individuoEvaluacion.getVinculo() != null && individuoEvaluacion.getVinculo().getIdVinculo() != null) {
	...
	public void setIdVinculoSeleccionado(Long idVinculoSeleccionado) {
		individuoEvaluacion.setVinculo((Vinculo) Util.buscarElemento(listAuxVinculo, idVinculoSeleccionado));
		/* @F4991 */}
	...
	public Date getFechaNacimiento() {
		/* @I4991 */return individuoEvaluacion.getFechaNacimiento();
		/* @F4991 */}


	public void setFechaNacimiento(Date fechaNacimiento) {
		/* @I4991 */individuoEvaluacion.setFechaNacimientoFlex(fechaNacimiento);
		/* @F4991 */}
	...
	public String getEmail() {
		/* @I4991 */if (individuoEvaluacion.getMails() != null && individuoEvaluacion.getMails().size() > 0) {
	...
	public void setEmail(String email) {
		this.email = email;
		/*
		 * Email e = getEmail(); if (e!=null && email!=null && !email.equals("")){ e.setEmail(email);
		 * 
		 * } this.email = email;
		 */
	}
	...
	public Long getIdEstadoCivilSeleccionado() {
		/* @I4991 */if (individuoEvaluacion.getEstadoCivil() != null && individuoEvaluacion.getEstadoCivil().getIdEstadoCivil() != null) {
			return individuoEvaluacion.getEstadoCivil().getIdEstadoCivil();
		}
		return new Long(0);
		/* @F4991 */}
	...
	public void setIdEstadoCivilSeleccionado(Long idEstadoCivilSeleccionado) {
		/* @I4991 */individuoEvaluacion.setEstadoCivil((EstadoCivil) Util.buscarElemento(this.listAuxEstadoCivil, idEstadoCivilSeleccionado));
		/* @F4991 */}
	...
	public Date getIngreso() {
		...
		try {
			return individuoEvaluacion.getActividad().getFechaIngresoFlex();
	...
	public String getAntiguedad() {
		/* @I4991 */try {
			return individuoEvaluacion.getActividad().getAntiguedad().toString();
		} catch (NullPointerException e) {
			return null;
			/* @F4991 */}
	}
	...
	/**************************** Nuevas Propiedades del Bean *********************/

	public boolean isBuscarIndividuo() {
		return buscarIndividuo;
	}


	public void setBuscarIndividuo(boolean buscarIndividuo) {
		this.buscarIndividuo = buscarIndividuo;
	}


	public boolean validarListGarantes(List listGarantes)
	{
		boolean result = true;
		Iterator iter = listGarantes.iterator();
		while (iter.hasNext()) {
			WrapperIndividuoEvaluacion indi = (WrapperIndividuoEvaluacion) iter.next();
			if (indi.getAccion().equalsIgnoreCase("ALTA") || indi.getAccion().equalsIgnoreCase("-"))
				result = false;
		}
		return result;
	}
	...
	/* @I4991 */public boolean validarAltaAdicional() {
		int count = 0;
		if (this.listAdicional != null) {
			for (Object obj : this.listAdicional) {
				WrapperIndividuoEvaluacion wIndividuo = (WrapperIndividuoEvaluacion) obj;
				if (!wIndividuo.getAccion().equalsIgnoreCase("BAJA")) {
					count += 1;
				}
			}
		}
	...
	private boolean objetoNegocioInconsistente(Negocio obj) {
		return obj == null || obj.getId() == null || obj.getId().equals(new Long(0));
	}
	...
	public class WrapperIndividuoEvaluacion
	{
		...
		public WrapperIndividuoEvaluacion(IndividuoEvaluacion individuoEva, String nvo, String activo)
		{
			this.individuoEvaluacion = individuoEva;
			this.nuevo = nvo;
			this.activo = activo;
			if (this.nuevo.equals("N"))
			{
				this.accion = (this.activo.equals("N") ? "BAJA" : "-");
			}
			else
			{
				this.accion = "ALTA";
		...
		/* @I4991 */public boolean getMostrarBotonEditar() {
			return !this.accion.equals("BAJA");
		}


		public String getMsgEliminarAdicional() {
			if (this.activo.equals("S"))
				return " Eliminar Adicional";
			else if (this.activo.equals("N"))
				return "Deshacer Baja Adicional";
			else
				return null;
		}


		public String getMsgEliminarGarante() {
			if (this.activo.equals("S"))
				return " Eliminar Garante";
			else if (this.activo.equals("N"))
				return "Deshacer Baja Garante";
			else
				return null;
		}


		public String getImgEliminarWrapper() {
			if (this.activo.equals("S"))
				return "/img/cat_act.gif";
			else if (this.activo.equals("N"))
				return "/img/nueva_entrada.gif";
			else
				return null;
