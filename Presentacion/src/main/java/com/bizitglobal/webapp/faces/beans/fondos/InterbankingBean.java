...
public class InterbankingBean extends BaseBean {
			...
			try {
				...
				while (iter.hasNext()) {
					...
					try {
						...
						if (chequeHistorial.getMovimientoMP() != null) {
							...
							if (target != null) {
								...
								//	linea.insert(104, Convertidores.completarIzquierda(liqComercio.getLiquidacion().getIdLiquidacion().toString(),
									linea.insert(104, Convertidores.completarIzquierda(liqComercio.getIdLiqComercio().toString(),
											new Character('0'), 12));
									comentario += liqComercio.getIdLiqComercio().toString() + " ";
									comentario += "OP" + movimiento.getIdMovimiento() + " ";
									linea.delete(130, 142);
									linea.insert(130,
											Convertidores.completarIzquierda(liqComercio.getCodComercio().getCodigoPosnet(), new Character('0'), 12));
									comentario +=  liqComercio.getCodComercio().getCodigoPosnet();
									...
									cuit = liqComercio.getCodComercio().getSucEmpresa().getEmpresa().getCuit().toString();
									nombreFantacia = liqComercio.getCodComercio().getSucEmpresa().getDescripcion();
									emails = fondosService.getChequeHistorialService()
											.buscarEmailComercio(liqComercio.getCodComercio().getIdCodComercio());
								} else {
									if (target.equals("PR")) {
										...
										comentario += comprobante.getNroLargo().toString() + " ";
										comentario += "OP" + movimiento.getIdMovimiento() + " ";
										linea.delete(130, 142);
										linea.insert(130, Convertidores.completarIzquierda(comprobante.getProveedor().getIdProveedor().toString(),
												new Character('0'), 12));
										comentario +=  comprobante.getProveedor().getIdProveedor().toString();
										// linea.append(Convertidores.completarIzquierda(" ", new Character(' '), 36));
										linea.delete(178, 189);
										linea.insert(178, comprobante.getProveedor().getCuit());
										cuit = comprobante.getProveedor().getCuit().toString();
										nombreFantacia = comprobante.getProveedor().getNombreFantasia();
										emails = fondosService.getChequeHistorialService()
												.buscarEmailProveedor(comprobante.getProveedor().getIdProveedor());
									}
								}
							}
						}
						...
						registro.setCbu(chequeHistorial.getCheque().getCbu());
						registro.setMonto(chequeHistorial.getCheque().getImporte().toString().replace(",", "."));
						registro.setNombreFantacia(nombreFantacia);
						registro.setCuit(cuit);
						registro.setOpcional(comentario);
						log.info("emails "+ emails );
						registro.setMails(emails);
	...
	public void generarReporteXls(){
		...
		String p1 = "?interbank=" + loteInterbank.getIdLoteInterbank() ;
		p2 = "&AExcel=excel";

		String page = request.getContextPath() + "/report/Transferencias.jrxml";
		
		ejecutarJavaScript("popup('" + page + p1+ p2 + "',700,400), 'titlebar=no';");
