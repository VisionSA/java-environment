...
public class LiquidacionComerciosParticularBean extends BaseBean {
	private static final Logger log = Logger.getLogger(LiquidacionComerciosParticularBean.class);
	...
	public LiquidacionComerciosParticularBean() {
		error.borrar();
	}
	...
	public String inicializar() {
		...
		return "liquidacionParticularComercios";
	}
	...
	public String buscarCodigoComercio() {
		...
		try {
			...
			List codComer = transaccionesService.getCodComercioService().getCodComercio(
					new Filtro("codigoPosnet", Filtro.LIKE, "" + codigoPosnetComercioBuscado));
			...
			filtro.agregarCampoOperValor("codComercio.idCodComercio", Filtro.IGUAL, comercioALiquidar.getIdCodComercio());
			filtro.agregarCampoComparacionNulo("liqComercio", Filtro.NULL);
			filtro.agregarCampoOperValor("conceptoDetalle.idConceptoDetalle", Filtro.IN, "17,359,525");
			filtro.agregarCampoOperValor("estadoImpacto", Filtro.LIKE, 'C');

			List ctacte = transaccionesService.getCtaCteComercioService().getCtaCteComercio(filtro);
				...
				if (tieneDatos) {
					...
					if (comercio.getExcluyeCargoPago().equals("N")) {
						if (esAcreditacionBancaria) {
							...
							generarCargoTransferenciaBancaria(comercio, conceptoCargoTransfBancaria, operador, liqC, wrapListaPrecio
							...
							if (esCheque) {
								...
								// String.valueOf(conDet.getCtacontablehaber()),"C", liqui.getFechaLiquidacion(), liqui.getFechaLiquidacion(),
								// null,conDet,liqC,null,operador.getCodigo(),new Long(0),new Long(0),null,new BigDecimal(cargoTransferenciaBancaria),
								// new Long(0),new Integer(1),"0", new Integer(1), new
								// Timestamp(Calendar.getInstance().getTime().getTime()),liqC.getCodComercio(), null,new Integer(1),null);
								generarCargoTransferenciaCheques(comercio, conceptoCargoTransfCheque, operador, liqC, wrapListaPrecio
										.getListaPrecioParaLiquidar().getFechaLiquidacion());
							} else {
								// ha de ser efectivo
								// creo el objeto CtaCteComercio, con el monto de transferencia bancaria. Se cobra un solo monto por transferencia
								// bancaria, indistintamente del numero de pagos.
								CtaCteComercio ctaCteComer = new CtaCteComercio(null, "N", String.valueOf(conDet.getCtacontabledebe()),
										String.valueOf(conDet.getCtacontablehaber()), "C", liqui.getFechaLiquidacion(), liqui.getFechaLiquidacion(),
										null, conDet, liqC, null, operador.getCodigo(), new Long(0), new Long(0), null, new BigDecimal(
												cargoTransferenciaBancaria).setScale(2, BigDecimal.ROUND_HALF_DOWN), new Long(0), new Integer(1),
										"0", new Integer(1), new Timestamp(Calendar.getInstance().getTime().getTime()), liqC.getCodComercio(), null,
										new Integer(1), null);
							}
						}
					}
					...
					lComPrincipal.prepararLiqComercio(iva, lComPrincipal.getTotalDeRetenciones(),
							BigDecimal.valueOf(cargos).negate().setScale(2, BigDecimal.ROUND_HALF_DOWN));
					...
					while (iterDeRetenciones.hasNext()) {
						...
						transaccionesService.getRetencionAComercioService().registrarRetencionAComercio(lComPrincipal.getCodComercio(), conDe,
								idTran, Session.getOperador(), lComPrincipal, wrapListaPrecio.getListaPrecioParaLiquidar().getFechaLiquidacion(),
								ret.getMonto());
					}
					...
					while (iterDeRetencionesSirtac.hasNext()) {
						...
						transaccionesService.getRetencionAComercioService().registrarRetencionAComercio(lComPrincipal.getCodComercio(),
								...
								wrapListaPrecio.getListaPrecioParaLiquidar().getFechaLiquidacion(), ret.getMontoRetenido());
					}
					
					// fin Sirtac

					// impacto en la cuentacte comercio el monto total de la liquidacion.
					generarTotalLiquidacionComercio(comercio, conceptoTotalLiquidacionComercio, operador, liqC, wrapListaPrecio
							.getListaPrecioParaLiquidar().getFechaLiquidacion());
					...
					pagosFuturos = liqui.getPagosDeLaLiquidacion(liqui.getMontoTotal(), lComPrincipal.getArancelNeto(),
							...
							lComPrincipal.getTotalDeRetenciones(), formaPago.getFormaPago().getDescripcion(), formaPago.getCodCuentaDeposito(),
							wrapListaPrecio.getListaPrecioParaLiquidar().getListaPrecio().getVersionVigente().isDifiereLiquidacionBoolean());
					...
					while (iterLiqC.hasNext()) {
						...
						while (lp.hasNext()) {
							...
							transaccionesService.getGestorLiquidacionComService().actualizarRegistrosCtaCteComercioParticular(
									liqLp.getListaPrecio().getIdListaprecios().intValue(), liquidacionComercio.getCodComercio().getIdCodComercio(),
									liquidacionComercio.getIdLiqComercio(), listaDeIdTransaccionesALiquidar);
						}
					}
	...
	public void borrar() {
		...
		tituloCorto = "Liquidación Particular de Comercios";
		codigoPosnetBuscado = "";
...
//
	...
	public void agregarRetenciones(LiqComercio liqComercio, CodComercio codComercio, Double cargos) {
		...
		try {
			...
			// BigDecimal ivaNeto = (liqComercio.getArancelNeto().add(liqComercio.getAceleramientoNeto())).multiply(new BigDecimal(0.21));
			BigDecimal ivaNeto = liqComercio.getIvaNeto();
	...
	public void agregarRetenciones(LiqComercio liqComercio, CodComercio codComercio, Double cargos,Double cargosSinIva) {
		...
		try {
			...
			log.info("entra cantidad operaciones: " + cantidadOperaciones);
			
			try {
				
				cantidadOperaciones  = listaDeIdTransaccionesALiquidar.size();	
				log.info("sale cantidad operaciones: " + cantidadOperaciones);
	...
	public String generar(LiqComercio liquidacionComercio, boolean esCheque) {
		...
		try {
			...
			gen.guardarReporte(page + p0 + p1 + p2 + p3 + p4 + p5 + p6 + p7 + p8 + p9 + p10 + p11 + p12 + p13 + p14 + p16 + retenciones + pagos
					+ periodos + p15);
			popupReport = "popup('" + page + "?" + p1.substring(1) + p2.replace("ƒ", "&") + p3.replace("ƒ", "&") + p4.replace("ƒ", "&")
					+ p5.replace("ƒ", "&") + p6.replace("ƒ", "&") + p7.replace("ƒ", "&") + p8.replace("ƒ", "&") + p9.replace("ƒ", "&")
					+ p10.replace("ƒ", "&") + p11.replace("ƒ", "&") + p12.replace("ƒ", "&") + p13.replace("ƒ", "&") + p14.replace("ƒ", "&")
					+ p16.replace("ƒ", "&") + retenciones.replace("ƒ", "&") + pagos.replace("ƒ", "&") + periodos.replace("ƒ", "&")
					+ p15.replace("ƒ", "&") + "',1000,600)";
			
			try {
				SimpleDateFormat formatoFechaS3 = new SimpleDateFormat("yyyy-MM-dd");	
				String bucket = "liquidacionesComercios/Particulares";
...
//		    	    multipart.addFormField("password", "test_psw");    	    
		    	    ...
		    	    // Add file
		    	    multipart.addFilePart("file", new File(nombreLiqS3));
		    	    // Print result
	...
	private String crearDirectoriosLiquidacion() throws Exception {
		...
		nombreLiq += File.separator + "Particulares";
	...
	public class WrapperListaPrecio {
		...
		public boolean liquidar(CodComercio codComer, Liquidacion liquidacion) {
			...
			if (listaPrecioParaLiquidar.getListaPrecio().getVersionVigente().isDifiereLiquidacionBoolean()) {
				...
				Set detallesLiq = transaccionesService.getGestorLiquidacionComService().liquidarCuotaParticular(
						this.listaPrecioParaLiquidar.getListaPrecio(), codComer.getIdCodComercio(),
						this.listaPrecioParaLiquidar.getListaPrecio().getVersionVigente().getIdListaPrecioVersion(), listaDeIdTransaccionesALiquidar,
						this.fechaLiq);
				...
				if (liquidacionCom == null) {
					// no existia aun
					if (detallesLiq != null) {
						liquidacionComercio = gestor.liquidarComerciosParticular(this.listaPrecioParaLiquidar, codComer, codComer.getSucEmpresa()
								.getEmpresa().getCuit().toString(), this.fechaLiq, detallesLiq);
					...
					if (detallesLiq != null) {
						...
						LiqComercioLP liqComLp = gestor.armarLPParticular(this.listaPrecioParaLiquidar, liquidacionCom.getCodComercio(),
