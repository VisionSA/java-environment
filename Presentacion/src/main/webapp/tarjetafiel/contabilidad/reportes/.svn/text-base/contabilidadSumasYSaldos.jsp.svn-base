...
<html lang="es">
<head>
    <title><h:outputText value="Tarjeta Fiel - Reporte (Contabilidad Sumas y Saldos)"/></title>
...
<body class="hold-transition skin-blue sidebar-mini" onbeforeunload="ShowWait('generalSumasYSaldosForm');" onload="${ReporteContabilidadSumasYSaldosBean.popupReport}">
...
<section class="content-header">
  <h1>
    ${ReporteContabilidadSumasYSaldosBean.tituloCorto}
    <small>${ReporteContabilidadSumasYSaldosBean.tituloLargo}</small>
  </h1>
</section>
...
<h:form id="generalBalanceForm">
    <x:panelLayout id="page" layout="#{globalOptions.pageLayout}"
        ...
        <f:facet name="body">
			...
			<%-- INCLUDE PARA LOS ERRORES --%> 	
				...
				<s:layoutingTitlePane id="filtroSumasYSaldos" label=" Filtro Reporte" 
					...
					<h:panelGrid id="filtroUno" columns="4" align="center">
			             ...
			             <h:selectOneMenu id="lstDeEjerciciosPorSucursal" value="#{ReporteContabilidadSumasYSaldosBean.idEjercicioSeleccionado}" binding="#{ReporteContabilidadSumasYSaldosBean.idEjercicioSeleccionadoItem}"
			       			styleClass="lista" onfocus="encender(this);" immediate="true" valueChangeListener="#{ReporteContabilidadSumasYSaldosBean.acomodarFechas}" onchange="submit();"
			       			onblur="apagar(this);" style=" width : 200px;">
			       			<f:selectItems value="#{ReporteContabilidadSumasYSaldosBean.ejerciciosSelectItem}" id="selectEjerDeSucum" />
			       		</h:selectOneMenu>
			       	...
			       	<f:verbatim>&nbsp;</f:verbatim>
						...
						<f:verbatim>&nbsp;</f:verbatim>
						<f:verbatim>&nbsp;</f:verbatim>
						<h:commandButton id="aExcel" action="#{ReporteContabilidadSumasYSaldosBean.generarAExcel}" 
											 value="Exportar A Excel" styleClass="btn btn-primary btn-flat" />
						
							<h:commandButton id="apdf" action="#{ReporteContabilidadSumasYSaldosBean.generar}" 
    ...
    <h:inputText id="FechaDesde" value="#{ReporteContabilidadSumasYSaldosBean.fechaDesde}" style="display: none;">
        <f:convertDateTime type="both" pattern = "dd/MM/yyyy HH:mm"/>
    </h:inputText>
    <h:inputText id="FechaHasta" value="#{ReporteContabilidadSumasYSaldosBean.fechaHasta}" style="display: none;">
        <f:convertDateTime type="both" pattern = "dd/MM/yyyy HH:mm"/>
    </h:inputText>
...
for(var i = 1; i < mainMenu__idJsp2_menu.length-1; i++) {
    ...
    document.getElementById("sideMenu").innerHTML += generateMenu(obj,"mainMenu__idJsp2_menu:A]\#{ReporteContabilidadSumasYSaldosBean.inicializar}") + `</li>`;
}
