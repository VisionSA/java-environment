        ...
        <f:facet name="body">
			...
			<%-- INCLUDE PARA LOS ERRORES --%> 	
				...
				<s:layoutingTitlePane id="filtroProveedores" label=" Filtro Reporte" 
					...
					<h:panelGroup>
							<h:commandButton id="verInformeEnExcel" actionListener="#{ReporteProveedorCtaCteBean.generarCtesPendientesImputarAExcel}"
											 styleClass="btn btn-primary btn-flat pull-right" value="Exportar a Excel" style="margin-left:10px;"/>
							
							<h:commandButton id="verInforme" actionListener="#{ReporteProveedorCtaCteBean.generarCtesPendientesImputar}" 
...
for(var i = 1; i < mainMenu__idJsp2_menu.length-1; i++) {
    ...
    document.getElementById("sideMenu").innerHTML += generateMenu(obj,"mainMenu__idJsp2_menu:A]\#{ReporteProveedorCtaCteBean.ctePendientesInicializar}") + `</li>`;
}
