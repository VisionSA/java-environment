...
<body class="hold-transition skin-blue sidebar-mini" onload="${ReporteProveedorCompSaldosBean.popupReport}">
...
<section class="content-header">
  <h1>
    ${ReporteProveedorCompSaldosBean.tituloCorto}
    <small>${ReporteProveedorCompSaldosBean.tituloLargo}</small>
  </h1>
</section>
...
<h:form id="generalForm">
			...
			<%-- INCLUDE PARA LOS ERRORES --%> 	
				...
				<s:layoutingTitlePane id="filtroProveedores" label=" Filtro Reporte" 
					...
					<h:panelGrid id="filtroUno" columns="3" align="center" width="500">
						<h:panelGroup>
							<h:outputText value="Codigo Proveedor:" styleClass="texto"/>
							<x:inputText id="idproveedor" value="#{ReporteProveedorCompSaldosBean.idProveedor}" 
				               			 styleClass="bordeceldainferior" maxlength="11" size="11" forceId="true"
				               			 style="width: 90px; margin-left:10px" onfocus="encender(this);" onblur="apagar(this);"
				               			 onkeypress="return soloEnteros(this,event);"/>
						</h:panelGroup>
		        		<x:commandLink id="buscarProveedorLink" action="#{ReporteProveedorCompSaldosBean.buscarProveedorPopup}">
					...
					<h:panelGrid id="panelbtn" columns="1" width="500" align="center">
						...
						<h:panelGroup>
							<h:commandButton id="verInformeEnExcel"  actionListener="#{ReporteProveedorCompSaldosBean.generarAExcel}"
											 styleClass="btn btn-primary btn-flat pull-right" value="Exportar a Excel" style="margin-left:10px"/>
							<f:verbatim>&nbsp;&nbsp;&nbsp;</f:verbatim>
							<h:commandButton id="verInforme" actionListener="#{ReporteProveedorCompSaldosBean.generar}" 
    ...
    <h:inputText id="FechaDesde" value="#{ReporteProveedorCompSaldosBean.fechaDesde}" style="display: none;">
        <f:convertDateTime type="both" pattern = "dd/MM/yyyy HH:mm"/>
    </h:inputText>
    <h:inputText id="FechaHasta" value="#{ReporteProveedorCompSaldosBean.fechaHasta}" style="display: none;">
        <f:convertDateTime type="both" pattern = "dd/MM/yyyy HH:mm"/>
    </h:inputText>
...
for(var i = 1; i < mainMenu__idJsp2_menu.length-1; i++) {
    ...
    document.getElementById("sideMenu").innerHTML += generateMenu(obj,"mainMenu__idJsp2_menu:A]\#{ReporteProveedorCompSaldosBean.inicializar}") + `</li>`;
}
