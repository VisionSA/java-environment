...
<html lang="es">
<head>
	<title><h:outputText value="Tarjeta Fiel | Listado de Pais"/></title>
</head>
<jsp:include page="/inc/includes.jsp" />
<body class="hold-transition skin-blue sidebar-mini" onbeforeunload="ShowWait('listadoPais');">
			...
			<f:facet name="body">
				<h:panelGroup id="body">
					...
					<h:panelGrid columns="1" align="center">
						...
						<s:layoutingTitlePane id="filtroPais" label=" Filtro Pais" containerNodeClass="contentTitlePane" labelNodeClass="labelTitlePane" >
							<h:panelGrid id="filtroUno" columns="6" align="center" width="600">
							
								<h:outputText value="Id País:" styleClass="texto"/>
								<h:inputText id="idpaisFiltro" value="#{PaisBean.idPais}"
								...
								action="#{PaisBean.listarPais}" title="Busca el pais seleccionado" styleClass="botones"/>
						...
						<c:if test="${!empty PaisBean.paisList}">
							<f:verbatim>
								<display:table id="listaPais" name="sessionScope.PaisBean.paisList"
									defaultsort="1" pagesize="25"
									class="table-bordered table-striped"
									requestURI="/tarjetafiel/general/listarPais.jsf"
									export="${lst:contains(requestScope.permisos,'exportacion')}" requestURIcontext="true" style="width: 900px;">

										<display:column property="idPais" title="Id País" sortable="true" class="tdB"/>
										<display:column property="nombre" title="Nombre País" sortable="true" class="tdA"/>
										<display:column style="width: 25px;" media="html">
											<c:choose>
												<c:when test="${lst:contains(requestScope.permisos,'edicion')}" >
													<a href="javascript:viewUser('${listaPais.idPais}','idPaisHidden');javascript:clickLink('listadoPais:editarPaisLink')">
														<img src='<%=request.getContextPath()%>/img/editar.gif' title='Editar País' border='0' />
													</a>
												</c:when>
												<c:otherwise>
										...
										<display:column style="width: 25px;" media="html">
											<c:choose>
												<c:when test="${lst:contains(requestScope.permisos,'baja')}" >
													<a href="javascript:viewUser('${listaPais.idPais}','idPaisHidden');javascript:clickLink('listadoPais:eliminarPaisLink')"
														onclick="return confirm('Confirma la eliminación del País.');">
														<img src='<%=request.getContextPath()%>/img/borrar.gif' title='Borrar País' border='0'/>
													</a>
												</c:when>
												<c:otherwise>
													<a href="#" onclick="return alert('No posee los permisos necesarios.');">
										...
										<display:setProperty name="paging.banner.item_name" value="Pais" />
										<display:setProperty name="paging.banner.items_name" value="Pais" />
							...
							<x:commandLink action="#{PaisBean.editarPais}" id="editarPaisLink" style="display: none;"/>
							<x:commandLink action="#{PaisBean.eliminarPais}" id="eliminarPaisLink" style="display: none;"/>

							<%-- Paso 3. Crear un input hidden para contener el id del elemento seleccionado --%>
							<x:inputHidden id="idPaisHidden" forceId="true" value="#{PaisBean.idPaisHidden}"/>
...
for(var i = 1; i < mainMenu__idJsp1_menu.length-1; i++) {
    ...
    document.getElementById("sideMenu").innerHTML += generateMenu(obj,"mainMenu__idJsp1_menu:A]\#{PaisBean.irAListarPais}") + `</li>`;
}
