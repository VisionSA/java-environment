...
package com.newmediateam.fileIO {
    ...
    public class MultiFileUpload {
        ...
        private function init():void{
	        ...
	        _nameColumn.headerText= "Archivo";
	        
	        _typeColumn.dataField = "type";
	        _typeColumn.headerText = "Tipo";
	        _typeColumn.width = 80;
	        
	        _sizeColumn.dataField = "size";
	        _sizeColumn.headerText = "Tamaño";
        ...
        ********************************************************/
        ...
        private function uploadFiles(event:Event):void{
            ...
            if (_files.length > 0){
                ...
                //_file.data is ByteArray
                 setupCancelButton(true);
                 
			    var bytesArr:ByteArray = _file.data as ByteArray;
			    var extracto:ExtractoBancario= new ExtractoBancario();
			    extracto.bytes = bytesArr; 
			    new ExtractoBancarioEvent(ExtractoBancarioEvent.GUARDAR_EXTRACTO_BANCARIO_EVENT,extracto).dispatch();
			    this.updateFilesUploaded();
            }
        }
        
        private function updateFilesUploaded():void{
            trace('updateFilesUploaded...');
        ...
        private function updateFilesArray(files:ArrayCollection,file:FileReference ):ArrayCollection{
        	var filesArray:ArrayCollection = new ArrayCollection();;	
        	var i:int;
       ...
       *********************************************************/
            ...
            var file:FileReference;   
	            ...
	            for (i=0;i < event.currentTarget.fileList.length; i ++){
	            	if (checkFileSize(event.currentTarget.fileList[i].size)){
           			file = FileReference(event.currentTarget.fileList[i]);
           			file.load();
           			_files.addItem(file);
        ...
        private function completeHandler(event:Event):void{
            trace('completeHander triggered');
            _files.removeItemAt(0);
            if (_files.length > 0){
            	_totalbytes = 0;
                uploadFiles(null);
