...
public class ManejoNumeros{
	...
	*
	* Esta sección permite transformar un valor numérico en la
	* correspondiente expresión literal que represente a dicho valor.
	*/
	
	public static String  getCadenaLetras (int número, long num_original) {
		...
		if (decena != 1  ||  unidad > 5){
			switch (unidad) {
		 		case 1: if (número <10 && num_original > 10){
		 					uu = "uno";
		 				}
		    	 		else{
		    	 			uu = "un";
	...
	public static String  getCadenaNumericaEnPalabras (long número) {
	 	String  cadena = "", c;
	 	int     x = 0, n;
	 	while (número > 0) {
	 		x++;
	    	...
	    	número /= 1000;	 
	    	cadena = getCadenaLetras(n, número) + c + cadena;
	 	}
	 	return  cadena; 
	}
