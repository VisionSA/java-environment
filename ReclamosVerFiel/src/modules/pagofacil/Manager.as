...
/* @I4662 */
		...
		public function procesarListaReclamoTiposTop():void{
			/* AlertOk.show("procesarListaTipoCobro"); */
			var evt:ProcesarPagoEvent = new ProcesarPagoEvent(ProcesarPagoEvent.PROCESAR_LISTA_RECLAMOS_TIPOS_TOP);
			dispatcher.dispatchEvent(evt);
			ControlBlock.getInstance().add();
		}
		...
		/* I8683 updateReclamo */
		public function updateReclamo(archivoReclamo:ReclamoCabecera,reclamoDetalle:ReclamoDetalle,subject:String,msq:String,nuevoImporte:Number):void{
			var evt:ProcesarPagoEvent = new ProcesarPagoEvent(ProcesarPagoEvent.UPDATE_RECLAMO);
			evt.archivoReclamo = archivoReclamo;
			evt.reclamoDetalle = reclamoDetalle;
			evt.sujeto = subject;
			evt.msq = msq;
			evt.importe = nuevoImporte;
			dispatcher.dispatchEvent(evt);
			ControlBlock.getInstance().add();
		}
		...
		public function generarReporteReclamoInicio(idReclamo:Number,destino:Number):void{			
			ControlBlock.getInstance().add();
		...
		public function generarReporteReclamoFin(idReclamo:Number,destino:Number):void{			
			ControlBlock.getInstance().add();
			var url,p1,p2,page : String;
			/*var array:Array = Application.application.url.split("/");*/
			/* uploadComp.uploadURL = "/" +array[3] + "/UploadFilePagoFacil"; */
