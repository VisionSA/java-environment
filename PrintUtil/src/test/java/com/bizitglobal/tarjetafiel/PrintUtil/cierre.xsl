...
<xsl:if test="@ajuste = 'true'">
    ...
    <xsl:value-of select="ajuste/medio/id"/><xsl:text> </xsl:text><xsl:value-of select="ajuste/medio/descripcion"/><xsl:text> </xsl:text><xsl:value-of select="ajuste/medio/importe"/><xsl:text> </xsl:text><xsl:value-of select="signo"/>
</xsl:if>
</xsl:if>
</xsl:for-each>
...
CHEQUES EN CAJA
Numero    Tipo    Importe
<xsl:for-each select="cierre/cheques/">
<xsl:value-of select="cheque/numero"/> <xsl:text>   </xsl:text><xsl:value-of select="cheque/tipo"/> <xsl:text>      </xsl:text><xsl:value-of select="cheque/importe"/>
</xsl:for-each>
</xsl:if>
</ticket>
</xsl:template>
