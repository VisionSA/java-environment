package managers
{
	...
	{   
		...
		[Bindable]public var varBloqueDesbloqueApp:String;
...
/*@F8156*/		}
		...
		if(_titular){
			var evt:ClienteManagerEvent = new ClienteManagerEvent(ClienteManagerEvent.BUSCAR_DESBLOQUEO_APP);
			evt.idCliente =  _titular.idCliente; 
			this.dispatcher.dispatchEvent(evt);				  
			}
		...
		public function haceBloqueDesbloqueoApp(cliente:Number,operador:Number):void{			
			if(_titular){
				ControlBlock.getInstance().add();			
				var evt:ClienteManagerEvent = new ClienteManagerEvent(ClienteManagerEvent.HACER_DESBLOQUEO_APP);
				evt.idCliente =  cliente; 
				evt.operador = operador;
				this.dispatcher.dispatchEvent(evt);	
			}
		}
	   ...
	   public function resultDesbloqueoApp(varBloqueDesbloqueApp:String):void{
		  // this.varBloqueDesbloqueApp = varBloqueDesbloqueApp;	
		   this.buscarTitularAdicionales(_titular.idCliente.toString(),BUSQ_POR_NRO_CUENTA,_titular);
		   ControlBlock.getInstance().remove();
		   
	   }
	   
	   public function resultBuscaDesbloqueoApp(varBloqueDesbloqueApp:String):void{
		   this.varBloqueDesbloqueApp = varBloqueDesbloqueApp;	
		   ControlBlock.getInstance().remove();
